...
impl<C: NetworkConnector<S>, S: NetworkStream> Client<C> {
    ...
    pub fn request<'b, B: IntoBody<'b>, U: IntoUrl>(&mut self, options: RequestBuilder<B, U>) -> HttpResult<Response> {
        // self is &mut because in the future, this function will check
        // self.connection_pool, inserting if empty, when keep_alive = true.

        let RequestBuilder { method, url, headers, body } = options;
...
pub struct RequestBuilder<'a, B: IntoBody<'a>, U: IntoUrl> {
    url: U,
...
impl<'a, B: IntoBody<'a>, U: IntoUrl> RequestBuilder<'a, B, U> {

    /// Creates a new `RequestBuilder`.
    pub fn new(method: Method, url: U) -> RequestBuilder<'a, B, U> {
        RequestBuilder {
            url: url,
            method: method,
            headers: None,
    ...
    pub fn body(mut self, body: B) -> RequestBuilder<'a, B, U> {
        self.body = Some(body);
        self
    }

    /// Add additional headers to the request.
    pub fn headers(mut self, headers: Headers) -> RequestBuilder<'a, B, U> {
        self.headers = Some(headers);
        self
    }

    /// Add an individual new header to the request.
    pub fn header<H: Header + HeaderFormat>(mut self, header: H) -> RequestBuilder<'a, B, U> {
        {
